//! 把扩展推过来的 Cookie 同步写到 `<data_dir>/douyin/cookies.json`，让 douyin crate 直接读用。
//!
...
pub async fn write_from_raw_header(data_dir: &Path, raw_header: &str) -> Result<serde_json::Value> {
    let paths = DouyinPaths::new(data_dir);
    paths.ensure_dirs()?;
