...
//! → 以 THS 兼容格式落盘到 `{workspace}/cookie/ths_cookies.json`。
//!
...
pub fn save(ws: &Path, records: &[CookieRecord]) -> Result<std::path::PathBuf> {
    let path = workspace::ths_cookies_path(ws);
...
/// 读 ths_cookies.json 给 UI / 主窗显示当前 THS 登录态状况。
pub fn status_report(ws: &Path) -> StatusReport {
    let path = workspace::ths_cookies_path(ws);
